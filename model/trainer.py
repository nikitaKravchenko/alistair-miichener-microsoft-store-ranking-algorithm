...
class UnifiedTrainer:
    ...
    def fit(
        ...
        if group_col is None:
            key = "__global__"
            model, importance_df, rmse = self._fit_single(df, (pretrained_models or {}).get(key))
    ...
    def _fit_single(
        ...
        try:
            ...
            if self.cfg.text_fields:
                # Persist TF-IDF and text settings
                setattr(model, "is_text_model_", True)
                setattr(model, "text_fields_", list(self.cfg.text_fields))
                setattr(model, "tfidf_", self._tfidf)          # primary
                setattr(model, "vectorizer_", self._tfidf)     # alias for convenience
            else:
                # Persist tabular helpers
                setattr(model, "is_text_model_", False)
                setattr(model, "categorical_features_", list(cat_feats or []))
                setattr(model, "label_encoders_", dict(self._label_encoders))
    ...
    def _build_text_matrix(self, df: pd.DataFrame, fields: List[str]) -> Tuple[pd.DataFrame, List[str]]:
        ...
        X_df = pd.DataFrame.sparse.from_spmatrix(
            X_sparse, columns=self._tfidf.get_feature_names_out()
        )
    ...
    def _train_with_optuna(
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            model.fit(
                X_train,
                y_train,
                categorical_feature=categorical_feature if categorical_feature else None
            )
            preds = model.predict(X_val)
